...
public class KdTree {
    ...
    public Iterable<Point2D> range(RectHV rect) {
        ...
        Queue<Point2D> residents = new Queue<>();
        Iterator<Point2D> iterator = points.keys(rect.xmin(), rect.ymin(), rect.xmax(), rect.ymax()).iterator();
        while (iterator.hasNext()) {
            Point2D point = iterator.next();
            if (rect.contains(point)) {
                residents.enqueue(point);
            }
        }
        return residents.isEmpty() ? new Stack<>() : residents;
    ...
    private class BST2d {
        ...
        private boolean get(Node x, Point2D key) {
            ...
            int xCmp = Point2D.X_ORDER.compare(x.key, key);
            int yCmp = Point2D.Y_ORDER.compare(x.key, key);
        ...
        private Node put(Node x, Point2D key) {
            ...
            int xCmp = Point2D.X_ORDER.compare(key, x.key);
            int yCmp = Point2D.Y_ORDER.compare(key, x.key);
        ...
        private void keys(Node x, Queue<Point2D> queue, Point2D lo, Point2D hi) {
            if (x == null) return;
            int xComplo = Point2D.X_ORDER.compare(lo, x.key);
            int yComplo = Point2D.Y_ORDER.compare(lo, x.key);
            int xComphi = Point2D.X_ORDER.compare(hi, x.key);
            int yComphi = Point2D.Y_ORDER.compare(hi, x.key);
        ...
        public Iterable<Point2D> keys(double xMin, double yMin, double xMax, double yMax) {

            Queue<Point2D> queue = new Queue<Point2D>();
            keys(root, queue, xMin, yMin, xMax, yMax);
            return queue;
        }
        private void keys(Node x, Queue<Point2D> queue, double xMin, double yMin, double xMax, double yMax) {
            if (x == null) return;

            int xComplo = x.key.x() > xMin ? 1 : x.key.x() < xMin ? -1 : 0;
            int yComplo = x.key.y() > yMin ? 1 : x.key.y() < yMin ? -1 : 0;
            int xComphi = x.key.x() > xMax ? 1 : x.key.x() < xMax ? -1 : 0;
            int yComphi = x.key.y() > yMax ? 1 : x.key.y() < yMax ? -1 : 0;
            ...
            if (cmplo >= 0 && cmphi <= 0) {
                queue.enqueue(x.key);
                keys(x.left, queue, xMin, yMin, xMax, yMax);
                keys(x.right, queue, xMin, yMin, xMax, yMax);
            } else if (cmphi > 0) {
                queue.enqueue(x.key);
                keys(x.left, queue, xMin, yMin, xMax, yMax);
            } else if (cmplo < 0) {
        ...
        public Node bottom(Point2D key) {
        ...
        private Node bottom(Node x, Point2D key) {
            if (x == null) return null;
            int cmp = 0;

            if (x.isX) cmp = Point2D.X_ORDER.compare(x.key, key);
            else cmp = Point2D.Y_ORDER.compare(x.key, key);
        ...
        public Node nearest(Point2D key) {
            if (key == null) throw new IllegalArgumentException("argument to ceiling() is null");
            if (isEmpty()) throw new NoSuchElementException("calls ceiling() with empty symbol table");
        ...
        private Node nearest(Node x, Point2D key, double rootDist) {
            if (x == null) return null;
            int cmp = 0;

...
원하는 목표는 사람의 최소인원이다.
우선
목표에 대한 최소인원은 1,2,3,... 순으로 나열되는 수열에서 선택하는 것이라 생각하면
최소인원은 허용값이 가장 작은 돌다리이고
최대인원은 가장 허용값이 큰 돌다리이다.

이러한 데이터 구조를 생각해보면 이분탐색으로 헤아려 보는 것이
작은 값부터 순차적으로 탐색하는 방식보다 빠를 수 있다.
"""
